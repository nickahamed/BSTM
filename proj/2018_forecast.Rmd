...
If the election were held today and assuming polling is not systematically flawed, there is a 100% chance that Democrats win over 50% of the popular vote, and more importantly, there is a 99% chance that they win a majority of seats in the House itself. However, we are still months away from the election. Simulations allow us to account for the variation we expect over time. Across 10,000 simulations of support __on election day__, Democrats win the popular vote in 63% of the simulations and a majority in the House in 55% of the simulations.
...
There are 198 seats where Democrats have more than a 60% chance of victory. 23 of these seats only lean towards the Democrat (less than an 80% chance of victory) and we must focus on protecting. An additional 88 seats lean Republican and are expansion seats if it looks like a wave year.
...
Sampling universes and pollsters who have polled in at least one of the last six election cycles use their previously estimated bias. See more [__here__](https://github.com/nickahamed/BSTM/blob/master/proj/pollster_bias.html). New pollsters have a weak prior described below. 

As of March 15, 2018, 134 polls from 19 pollsters contacting 166k respondents were used. These are the 5 largest pollsters. See Appendix B for full details.
...
New pollsters/universes that have not previously polled have the following priors. 
$$\ \ \ \delta_j \sim \mathcal{N}(0, (0.2/1.96)^2),\ \ \ \theta_k \sim \mathcal{N}(0, (0.2/1.96)^2)$$
For pollster biases ($\delta$), I start with a prior that there is no bias with a standard deviation to reflect that bias is 95% of the time within 20pp in either direction; likewise, I start with a prior for bias from sampling universe ($\theta$) that is the same. 
...
deltas <- rbind(deltas, data.frame(delta_cycle = rep(2018, length(setdiff(polls$pollster, pollster_lkup$pollster))),
                                   delta_pollster = setdiff(polls$pollster, pollster_lkup$pollster),
                                   delta_mu = rep(0, length(setdiff(polls$pollster, pollster_lkup$pollster))),
                                   delta_sigma2 = rep(sigma2, length(setdiff(polls$pollster, pollster_lkup$pollster))),
                                   pollster_raw = unique(polls$pollster_raw[polls$pollster %in% setdiff(polls$pollster, pollster_lkup$pollster)])))
