...
Second, while there appears to be significant variation over time in the polls, the trendlines are much smoother. In fact, week-over-week, the models estimate 95% of movement is less than 1.3pp. The model used here has one parameter for week-to-week movement, so it averages over big swings (see weeks 63-50 in 2014) and small, incremental changes (see weeks 100-63 in 2014). A model that separates out these trends might better reflect some swings in the polls, and identify which ones are noise. 
...
<br>
<br>
Third, some cycles' models are more accurate than others. The table below shows the percentage point error of each final forecast. In 2006, 2014 and 2016, our models were very accurate. In 2008, 2010 and 2012, they missed by a few percentage points, always underestimating Democratic support. 

<div style= "float:left;position: relative; top: 10px;">
![](figures/final_est_vs_seats.png){ width=2in }
</div>

The model that adapts for both past pollster and sampling universe bias is more accurate than a model that accounts for just pollster bias or no bias at all. Over the six cycles analyzed here, there is a 0.1pp reduction in bias compared to the model with no bias and 0.2pp over the model that just accounts for pollster bias. Likewise, the model that accounts for no bias sees larger week-to-week movement by about 0.1pp. 

Lastly, we are interested in the relationship between the final forecasts and the actual share of seats won. Becuase of our winner-take-all and gerrymandered system, the popular vote rarely translates directly into share of seats won. A simple linear regression shows that 67% of the variation in seats won over the last 6 elections is explained by the forecasts produced here. Additionally, Democrats need to be forecasted to win more than 50% of the popular vote to win 50% of the seats.

##Conclusions
Using past public polling data and election results, I generated bias estimates for each pollster and sampling universe activly polling the generic Congressional ballot over the past six cycles. Generally, pollsters are consistently biased by less than 1pp, but a few exceed this theshold. Of the pollsters analyzed, I find PPP is the least biased pollster. Likewise, I find that likely voter sampling universes are less biased than registered voter or adult universes. All three universes tend to overestimate the true level of Democratic support. 

I then use these (admittedly overfit) estimates of bias to generate forecasts for the six cycles analyzed. The final forecasts had a mean absolute error of about 1.5pp, though five-of-six correctly predicted the winner. These forecasts are fairly predictive of the final seat share Democrats would win.
...
$\sigma^2_i$ just follows the formula for standard deviation of a sample. As a prior for the starting true value of support ($\alpha_1$), I use a uniform distribution over the minimum and maximum actual vote share of Democrats in the six elections analyzed. Lastly, as a prior for the true standard deviation of support ($\omega$), I use a uniform distribution between 0 and 0.1. A value of 0.1 would reflect that 95% of week-to-week movement is within about 2pp in either direction, a fairly weak assumption. These priors are similar to [__Strauss (2007)__](http://www.mindlessphilosopher.net/princeton/strauss_reverse%20random%20walk.pdf) 
...
pollster_bias <- ggplot(deltas, aes(x=pollster_raw, y=delta_mu)) + 
  ...
  guides(color = F) +
  xlab("Pollster") +
  scale_y_continuous(name = "Bias Estimate", labels = scales::percent, limits = c(-0.045,0.055)) + 
...
pollster_cycle <- ggplot(temp, aes(x=delta_cycle, y=delta_mu, group = pollster_raw)) + 
  ...
  theme_bw() +
  xlab("Cycle") +
  scale_y_continuous(name = "Bias Estimate", labels = scales::percent, limits = c(-0.03,0.03)) + 
...
univ_bias <- ggplot(thetas, aes(x=theta_univ, y=theta_mu)) + 
  ...
  scale_y_continuous(name = "Bias Estimate", labels = scales::percent, limits = c(0,0.04)) + 
  theme(axis.text.x = element_text(angle = 90, hjust = 1, size = 10, vjust = 0.5))
...
univ_cycle <- ggplot(temp, aes(x=theta_cycle, y=theta_mu, group = theta_univ)) + 
  ...
  theme_bw() +
  xlab("Cycle") +
  scale_y_continuous(name = "Bias Estimate", labels = scales::percent, limits = c(-0.01,0.04)) + 
...
lm_fit <- ggplot(final_ests, aes(x = twoway_seat, y = iter_mean)) + 
  ...
  coord_flip() + 
  theme_bw() + 
  scale_y_continuous(name = "Election Forecast", limits = c(0.4, 0.61), labels = scales::percent) + 
