...
```{r, echo = FALSE}
ggplot(deltas, aes(x=pollster_raw, y=delta_mu)) + 
...
ggplot(temp, aes(x=delta_cycle, y=delta_mu, group = pollster_raw)) + 
...
```{r, echo = FALSE}
ggplot(thetas, aes(x=theta_univ, y=theta_mu)) + 
  geom_point() +
  theme_bw() +
  guides(color = F) +
  geom_hline(yintercept = 0, linetype = "dashed") + 
...
ggplot(temp, aes(x=theta_cycle, y=theta_mu, group = theta_univ)) + 
...
convergence_diagnostics <- function(chains = 4, 
                                    ...
                                    iter = 2500000, 
                                    data_jags) { 
  message(paste0("Sample convergance diagnostics for ", unique(data_jags$cycle), " parameters:"))
  ...
  plot(mod_res)
  print(gelman.diag(mod_res))
  print(autocorr.diag(mod_res))
  autocorr.plot(mod_res)
  effectiveSize(mod_res)
}
```
...
for(cycle in res$cycle) {
  data_jags <- data_prep(data = polls, res = res, year = cycle)
  data_jags <- bias_priors(data_jags = data_jags, deltas = deltas, thetas = thetas)
  convergence_diagnostics(data_jags = data_jags)
  mod_res <- run_model(data_jags = data_jags)
...
#Final estimate of pollster bias and error
print(deltas) #Flextable

#Estimate for every cycle and error
print(deltas_all) #Flextable

#Final estimate of universe bias and error
