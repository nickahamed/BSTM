...
for(cycle in res$cycle) {
  ...
  deltas <- new_priors$deltas %>% 
    arrange(delta_mu) %>%
    mutate(delta_pollster = factor(delta_pollster, levels = delta_pollster[order(delta_mu)]))
...
```{r, echo = FALSE}
ggplot(deltas, aes(x=delta_pollster, y=delta_mu)) + 
  geom_point(aes(color = delta_mu)) +
  geom_hline(yintercept = 0, linetype = "dashed") + 
  scale_color_continuous(low="red", high="blue") +
  scale_x_discrete(labels = stringr::str_to_title(str_replace(deltas$delta_pollster, "_", " "))) +
  theme_bw() +
  guides(color = F) +
  xlab("Pollster") +
...
ggplot(temp, aes(x=delta_cycle, y=delta_mu, group = delta_pollster)) + 
  geom_point(alpha = 0.9, aes(color = delta_pollster)) +
